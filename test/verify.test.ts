...
test('verify-decode-jws-compact-0', async () => {
    const verificationRecord = await api.verify(validCjws, { directory: testDirectory });
    expect(verificationRecord).toMatchObject(testVerificationRecord);
});

test('verify-decode-shc', async () => {
    const verificationRecord = await api.verify(validShc, { directory: testDirectory });
    expect(verificationRecord).toMatchObject(testVerificationRecord);
});

test('verify-decode-jws-compact-altered-signature', async () => {
    const badSignature = validCjws.replace('.qI', '.qJ');
    const verificationRecord = await api.verify(badSignature, { directory: testDirectory });
